...
export function useApprovalState(tokenInfo: any): [ApprovalType, () => Promise<void>] {
  ...
  useEffect(() => {
    async function getCurrentAllowance() {
      if (account) {
        const userAllowance = await tokenContract?.allowance(account, DATABANK_CONTRACT);
  ...
  const approveSpend = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const approvalTx = await tokenContract?.approve(DATABANK_CONTRACT, amountToSpend)
